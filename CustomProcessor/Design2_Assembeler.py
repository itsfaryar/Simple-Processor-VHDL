...
def getIn6bitBinaryFormat(Integer: int):
    ...
    while(len(binary) < 7): binary = "0" + binary
    return binary
def getCodeInBinaryFormat(file_path):
    ...
    try:
        for line in f:
            ...
            if parts[0] == "LOAD":
               output.append( "000" + reg_dic[parts[1]] + "01")
               output.append(getIn6bitBinaryFormat(int(parts[2])))
            elif parts[0] == "ADD":
                output.append("001" + reg_dic[parts[1]] + reg_dic[parts[2]])
            elif parts[0] == "SUB":
                output.append("010" + reg_dic[parts[1]] + reg_dic[parts[2]])
            elif parts[0] == "JNZ":
                output.append("011" + reg_dic[parts[1]] + "00")
                output.append(getIn6bitBinaryFormat(int(parts[2])))
            elif parts[0] == "MULT":
                output.append("100" + reg_dic[parts[1]] + reg_dic[parts[2]])
            elif parts[0] == "HLT":
